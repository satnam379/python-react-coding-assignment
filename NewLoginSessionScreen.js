...
import { View, Text, LogBox, StyleSheet, ImageBackground, BackHandler, ActivityIndicator, TouchableOpacity, Platform, Image } from 'react-native';
...
const NewLoginSessionScreen = props => {
    async function saveUserInformationToLocalStorage(key, value) {
            ...
            (response) => {
                try {
                    ...
                    if (Platform.OS === 'ios') {
                        ...
                        props.navigation.navigate(GlobalData.constData.testBandScoreRoute, { testIsReviewed: IsPracticeTestFinalized, applicantId: userInformation.userInfo.id, instituteId: userInformation.userInfo.instituteId, itemId: iositemId, throughUserDashBoard: true, examId: examId, route: global.globalRoute, practiceTitle: PracticeTitle });
                    }
                } catch (error) {
                    console.log(error);
                }
    ...
    const logout = async () => {
        ...
        try {
            ...
            if (logoutUrl.type !== 'cancel') {
                // props.navigation.navigate('logoutSuccess', { load: true })
                ...
                const response = await fetch(logoutUrls,
                    {
                        method: 'POST',
                        headers: {
                            'Accept': 'application/json',
        ...
        try {
            if (user.decoded) {
                ...
                if (decodedJwt.exp * 1000 < Date.now()) {
                    ...
                    var res = await fetch(GlobalData.constData.apiUrl + GlobalData.constData.userInformation + decoded.sub + '&deviceId=' + expoPushToken,
                        {
                            headers: {
                                'Accept': 'application/json',
                                'Authorization': `Bearer ${token}`
                    ...
                    if (json_data.ieltsInformation !== null) {
                        let dateTime = DateTime(json_data.ieltsInformation.ieltsExamDate)
    ...
    const readTokenFromStorage = async () => {
        ...
        if (tokenConfig) {
            ...
            if (tokenResponse.shouldRefresh()) {
                const refreshConfig = { clientId: auth0ClientId, refreshToken: tokenConfig.refreshToken }
    ...
    }, [result, notification, expoPushToken])

    useEffect(() => {
        fetchUserInfo(user.decoded)
    }, [result, expired, notification, expoPushToken, user])

    const ShowContinuePage = (props) => {
        return (
            <View style={{ flex: 1, justifyContent: 'center' }}>
                <StatusBar backgroundColor={Colors.colorCodes.transparent} translucent style="dark" />
                <View contentContainerStyle={{ flexGrow: 1, height: '100%', marginBottom: '30%' }}>
                    <ImageBackground style={styles.bgImage} source={GlobalData.constData.BackGroundImage}>
                        {props.showLoader === true ? (
                            <View style={styles.imageContainer}>
                                ...
                                <View style={styles.indicatorView}>
                                    <View style={{ justifyContent: 'center', alignItems: 'center' }}>
                                        <ActivityIndicator size="large" color={Colors.colorCodes.orange} />
                                    </View>
                                    <CountDown
                                        digitStyle={{ borderWidth: 0, fontFamily: FontFamilies.fontFamilies.montserratBold }}
                            ...
                            <View style={styles.buttonView}>
                                <TouchableOpacity activeOpacity={0.6} style={styles.button} onPress={() => { promptAsync() }
                                } >
    ...
    if (user.jwtToken) {
        ...
        if (!user.jwtToken && result === null) {
            return (
                <ShowContinuePage showLoader={true} text={''} />
            )
        }
        else if ((!user.jwtToken && result.type != 'dismiss' && result.type != 'cancel' && result.type != 'locked')) {
            return (
                <ShowLoader />
            )
        }
    ...
    if (!isAuthReady && result.type != 'success') {
        return (
            <ShowContinuePage showLoader={false} text={'Please click here to continue'} />
        );
    }
    return (
        <ShowContinuePage showLoader={false} text={'Welcome to LIAH, Please click here to continue'} />
    );
}
...
const styles = StyleSheet.create({
    ...
    subHeading: {
        fontSize: 12,
        color: Colors.colorCodes.lightGrey,
        marginBottom: 10,
...
export default NewLoginSessionScreen;